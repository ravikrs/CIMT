package de.rwth.i9.cimt.algorithm.graph;
...
public class HITS {
	...
	// 3 argument constructor to initialize class variables with provided
	// command line arguments
	HITS(int iter, int initval, String filename) {
		this.iter = iter;
		...
		/*
		 * for(int i = 0; i < n; i++) { System.out.println(); System.out.print(i
	...
	public void hitsAlgo3416() {
		...
		double[] aprev = new double[n]; // last iterations values of a, used for
										// convergence
		double[] hprev = new double[n]; // last iterations values of h, used for
										// convergence

		// If the graph has N greater than 10, then the values for iterations,
		// initialvalue revert to 0 and -1 respectively
		if (n > 10) {
			iter = 0;
			for (int i = 0; i < n; i++) {
				h[i] = 1.0 / n;
			...
			for (int l = 0; l < n; l++) {
				System.out.printf(" A/H[%d]=%.6f/%.6f\n", l, Math.round(a[l] * 1000000.0) / 1000000.0,
						Math.round(h[l] * 1000000.0) / 1000000.0);
			}
			return;
		}
		...
		for (int i = 0; i < n; i++) {
			System.out.printf(" A/H[%d]=%.6f/%.6f", i, Math.round(a0[i] * 1000000.0) / 1000000.0,
		...
		if (iter != 0) {
			for (int i = 0; i < iter; i++) { // iteration starts
				...
				for (int l = 0; l < n; l++) {
					System.out.printf(" A/H[%d]=%.6f/%.6f", l, Math.round(a[l] * 1000000.0) / 1000000.0,
							Math.round(h[l] * 1000000.0) / 1000000.0);
				}

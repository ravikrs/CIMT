...
import de.rwth.i9.cimt.algorithm.kpextraction.textrank.WordNet;
...
public class CimtWordNetResource extends AbstractResource {
	...
	public CimtWordNetResource(String wordNetPropertiesFile) throws LexicalSemanticResourceException {
		try {
			WordNet.buildDictionary(wordNetPropertiesFile);
			this.dict = WordNet.getDictionary();
			this.v = dict.getVersion();
			setIsCaseSensitive(isCaseSensitive); // zhu
		} catch (Exception e1) {
			logger.info("Could not initialize  CimtWordNetResource");
			throw new LexicalSemanticResourceException(e1);
		}
	}
